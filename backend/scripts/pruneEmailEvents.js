...
const parseArgs = () => {
  ...
  const out = {
    days: Number(process.env.EMAIL_EVENTS_RETENTION_DAYS || 90) || 90,
    dryRun: false,
  };
...
const main = async () => {
  ...
  console.log(`Pruning emailEvents older than ${days} day(s) (cutoff ${new Date(cutoffMs).toISOString()})${dryRun ? ' [DRY RUN]' : ''}`);
  ...
  while (true) {
    ...
    if (dryRun) {
      ...
      // Prevent infinite loops in dry-run: just one page is enough.
      break;
    }
    ...
    // Yield to avoid hammering Firestore.
    await new Promise((r) => setTimeout(r, 50));
  }

  console.log(`Done. ${dryRun ? 'Would delete' : 'Deleted'} ${deleted} emailEvents.`);
};

main().catch((err) => {
  console.error('pruneEmailEvents failed:', err?.message || String(err));
  process.exitCode = 1;
});
