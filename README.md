...
- Backend calls use an authenticated fetch wrapper that will attach `Authorization: Bearer <Firebase ID token>` when a Firebase user is signed in.
...
- To require auth on protected endpoints, set `REQUIRE_FIREBASE_AUTH=true` in `backend/.env`.
...
### Firestore index warning (memberships.user_id)

If you see a warning like:

`[vaults] memberships query needs a Firestore collectionGroup index on memberships.user_id ...`

deploy the included Firestore index configuration:

- `npm install -g firebase-tools`
- `firebase login`
- `firebase use lamb-e6cd0` (or your Firebase project)
